"""裁切音频：去掉开头静音、限制长度，让音效更早出现（用于按键提示音）。"""
...
SRC = Path(r"C:\Users\Junhong\Music\8月8日.mp3")
OUT = Path(__file__).resolve().parent.parent / "app" / "assets" / "8月8日_裁剪.wav"
WINDOW_SEC = 0.005
THRESHOLD = 0.04
HEAD_PAD_SEC = 0.02
MAX_LEN_SEC = 1.5
...
def main() -> None:
    ...
    trim_end = min(onset_sec + MAX_LEN_SEC, end_sec + TAIL_PAD_SEC)
    s0 = int(start * sample_rate)
    s1 = int(trim_end * sample_rate)
