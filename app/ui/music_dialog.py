...
class MusicPlayerDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.now_label.setStyleSheet("font-size:15px; font-weight:bold;")
        ...
        for b in (self.play_btn, self.pause_btn, self.stop_btn, self.prev_btn, self.next_btn):
    ...
    def play_current(self) -> None:
        ...
        if not path:
            return
        player, _ = self.mw._ensure_music_player()
        if player is None:
            return
    ...
    def _on_state(self, state) -> None:
        if state == QMediaPlayer.PlayingState:
            self.now_label.setText(f"正在播放：{self.playlist.currentItem().text() if self.playlist.currentItem() else ''}")
